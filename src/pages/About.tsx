  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-light-bg to-white pt-24 pb-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-20">
          {stats.map((s) => (
            <motion.div key={s.label} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
              className="bg-white border border-neutral-200/60 shadow-sm p-6 rounded-2xl text-center">
              ...
              <p className="text-2xl font-extrabold text-neutral-900 font-display">{s.value}</p>
              <p className="text-xs text-neutral-500 font-display uppercase tracking-wider mt-1">{s.label}</p>
            </motion.div>
          ))}
        </div>
        ...
        {/* Values */}
        <div className="mb-20">
          <h2 className="font-display text-xl font-bold text-neutral-900 uppercase tracking-widest text-center mb-10">Our Values</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {values.map((v) => (
              <motion.div key={v.title} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
                className="bg-white border border-neutral-200/60 shadow-sm p-6 rounded-2xl">
                <h3 className="font-display text-sm font-bold text-neutral-800 mb-2">{v.title}</h3>
        ...
        {/* CTA */}
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center bg-neutral-900 rounded-2xl p-10 sm:p-14">
          <h2 className="font-display text-2xl sm:text-3xl font-extrabold text-white mb-3">Ready to hit the road?</h2>
          <p className="text-neutral-400 text-sm mb-6 max-w-lg mx-auto">Browse our collection of premium vehicles and find the perfect ride for your next journey.</p>
