    ...
    <footer className="bg-white border-t border-neutral-200/60 pt-16 pb-8 text-neutral-500 relative z-10">
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-10 lg:gap-8 pb-12">
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="flex gap-3">
            {[Globe, Play, MessageCircle].map((Icon, i) => (
              <a key={i} href="#" className="p-2 border border-neutral-200 hover:border-neutral-400 hover:text-neutral-800 transition-colors bg-neutral-50 rounded-lg"><Icon size={15} /></a>
            ))}
          </div>
        </div>
        {[
          ...
          <div key={col.title} className="space-y-3">
            <h4 className="font-display text-[10px] font-bold text-neutral-800 uppercase tracking-widest">{col.title}</h4>
        ...
        <div className="space-y-3">
          <h4 className="font-display text-[10px] font-bold text-neutral-800 uppercase tracking-widest">Newsletter</h4>
          ...
          {subscribed ? <div className="text-xs text-accent-blue">✓ Access granted.</div> : (
            <form onSubmit={handleSubscribe} className="flex border border-neutral-200 focus-within:border-accent-blue transition-colors rounded-lg overflow-hidden">
              <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="EMAIL"
                className="w-full bg-neutral-50 text-[10px] uppercase text-neutral-800 px-3 py-2 outline-none font-sans" />
              <button type="submit" className="px-3 border-l border-neutral-200 hover:text-neutral-800 hover:bg-neutral-100 transition-colors cursor-pointer"><ArrowRight size={14} /></button>
            </form>
          )}
        </div>
      </div>
